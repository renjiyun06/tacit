...
// Pure interpreter that walks the AST. No LLM here — we verify the
...
// Ask the LLM to parse the expression into an AST. The discriminated
// union type is the entire spec — no prompt, no schema.
const ast: Expr = await ai(code);
